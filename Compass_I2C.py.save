...
class COMPASS:
    ...
    def calibrate_comp(self):
        ...
        for i in range(0, 500):
            x_out, y_out, z_out = self.readall_comp()
    ...
    def readall_comp(self):

        raw_comp_data = self.bus.read_i2c_block_data(i2c_address, comp_address, 9)
    ...
    def comp_rotation(self, x_offset, y_offset):
        comp_scaled_x, comp_scaled_y, comp_scaled_z = self.readall_comp()

        x_out = (comp_scaled_x - x_offset) * comp_scale
        y_out = (comp_scaled_y - y_offset) * comp_scale

        return get_CompassRotation(x_out, y_out)
